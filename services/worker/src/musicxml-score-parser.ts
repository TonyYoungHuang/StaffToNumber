...
function text(value: unknown): string | undefined {
  ...
  return isObject(value) ? text(value["#text"]) : undefined;
}
...
function isPitchStep(value: string | undefined): value is ScorePitchStep {
  return value === "A" || value === "B" || value === "C" || value === "D" || value === "E" || value === "F" || value === "G";
}

function parseTitle(score: XmlObject, fallbackTitle: string) {
  return text(child(child(score, "work"), "work-title")) ?? text(child(score, "movement-title")) ?? fallbackTitle;
}

function parseComposer(score: XmlObject) {
  for (const creator of asArray(child(child(score, "identification"), "creator"))) {
    if (text(attr(creator, "type")) === "composer") {
      return text(creator);
    }
  }
  return undefined;
...
function parseParts(score: XmlObject, warnings: string[]): ScorePart[] {
  ...
  if (scoreParts.length === 0) {
    warnings.push("Audiveris MusicXML did not include score-part entries.");
  }

  return scoreParts.map((part, index) => {
    const id = text(attr(part, "id")) ?? `P${index + 1}`;
    return {
      id,
...
function orderedChildren(nodes: unknown, key: string): unknown[] {
  ...
  for (const node of nodes) {
    if (isObject(node) && Array.isArray(node[key])) return node[key] as unknown[];
  }
  return [];
}
...
function orderedChildText(nodes: unknown, key: string) {
  for (const value of orderedChildren(nodes, key)) {
    if (isObject(value) && value["#text"] !== undefined) return text(value["#text"]);
  }
  return undefined;
}

function parseStaffGroups(musicXml: string, warnings: string[]): ScoreStaffGroup[] {
  const document = orderedParser.parse(musicXml) as unknown;
  const partListChildren = orderedChildren(orderedChildren(document, "score-partwise"), "part-list");
  const activeGroups = new Map<string, ScoreStaffGroup>();
  const completedGroups: ScoreStaffGroup[] = [];
  let generatedGroupNumber = 1000;
  ...
  for (const entry of partListChildren) {
    ...
    if (Array.isArray(entry["score-part"])) {
      ...
      if (partId) for (const group of activeGroups.values()) group.partIds.push(partId);
    }
  }
...
function parseAttributes(measure: unknown): ScoreMeasureAttributes | undefined {
  ...
  const clefs = asArray(child(attributes, "clef"))
  ...
  const parsed: ScoreMeasureAttributes = {
    divisions: numberFrom(child(attributes, "divisions")),
    key: key
      ? {
          fifths: numberFrom(child(key, "fifths"), 0) ?? 0,
          mode: text(child(key, "mode")),
        }
      : undefined,
    time:
      text(child(time, "beats")) && text(child(time, "beat-type"))
        ? {
            beats: text(child(time, "beats"))!,
            beatType: text(child(time, "beat-type"))!,
          }
        : undefined,
  ...
  return asArray(child(measure, "harmony"))
    .map((harmony, index) => {
      ...
      const parsedHarmony: ScoreHarmony = {
        ...
        rootStep,
        rootAlter: numberFrom(child(root, "root-alter"), 0) ?? 0,
...
function parseLayoutHint(measure: unknown, measureId: string): ScoreLayoutHint | undefined {
  ...
  const staffDistance = numberFrom(child(asArray(child(print, "staff-layout"))[0], "staff-distance"));
  const newSystem = text(attr(print, "new-system")) === "yes";
  const newPage = text(attr(print, "new-page")) === "yes";
  if (!newSystem && !newPage && measureWidth === undefined && staffDistance === undefined) return undefined;
  return {
    id: `${measureId}-layout`,
...
function parsePitch(note: unknown): ScorePitch | undefined {
  ...
  if (!isPitchStep(step) || octave === undefined) {
    return undefined;
  }

  return {
    step,
    alter: numberFrom(child(pitch, "alter"), 0) ?? 0,
    octave,
  };
}
...
const BEAM_TYPES = new Set<ScoreBeam["type"]>(["begin", "continue", "end", "forward-hook", "backward-hook"]);
const ORNAMENT_TYPES = ["trill-mark", "turn", "delayed-turn", "inverted-turn", "mordent", "inverted-mordent", "tremolo"] as const;
...
function parseBeams(note: unknown, eventId: string, warnings: string[]): ScoreBeam[] {
  return asArray(child(note, "beam"))
    .map((beam, index) => {
      ...
      return { id: `${eventId}-beam-${number}-${index + 1}`, number, type } satisfies ScoreBeam;
    })
  ...
  return asArray(child(note, "notations"))
    ...
    .map<ScoreTuplet | undefined>((tuplet, index) => {
      ...
      if (type !== "start" && type !== "stop") {
        warnings.push(`Unsupported tuplet type at ${eventId}; it was not imported.`);
        return undefined;
      }
    ...
    asArray(child(notation, "ornaments")).flatMap((ornaments, ornamentsIndex) => {
      if (isObject(ornaments)) {
        for (const key of Object.keys(ornaments)) {
          if (!key.startsWith("@_") && key !== "#text" && !(ORNAMENT_TYPES as readonly string[]).includes(key)) {
            warnings.push(`Unsupported ornament ${key} at ${eventId}; it was recorded as an import warning.`);
          }
        }
      }
      return ORNAMENT_TYPES.flatMap((type) =>
...
export function parseAudiverisMusicXmlToScoreJson(input: {
  musicXml: string;
  title: string;
  sourceFileId: string;
  ...
  const parsed = parser.parse(input.musicXml) as XmlObject;
  const score = child(parsed, "score-partwise");
  const warnings: string[] = [];

  if (!isObject(score)) {
    throw new Error("Audiveris did not produce a supported score-partwise MusicXML document.");
  }
  ...
  const partsWithCounts = parts.map((part) => ({
    ...
    staffCount: Math.max(
      1,
      ...measures.filter((measure) => measure.partId === part.id).map((measure) => measure.attributes?.staves ?? 1),
    ),
