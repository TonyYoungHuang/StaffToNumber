...
export const zhTWSupportLegal = {
  ...
  privacy: {
    ...
    sections: [
      ...
      { title: "Cookie 與分析", points: ["功能性語言 Cookie 會記住所選介面語言。", "Cloudflare Web Analytics 的真實使用者監測（RUM）已全站啟用，用於衡量頁面瀏覽與載入效能；它不使用 Cookie、不讀取瀏覽器儲存，並在 Cloudflare 邊緣節點捨棄訪客 IP 位址。", "只有在明確同意且正式環境分析已啟用時，才會載入 GA4 或 Microsoft Clarity。", "拒絕需要同意的分析不會失去公開內容或產品流程的存取權。"] },
